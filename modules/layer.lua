...
function luagimp.layer.new(model)
    ...
    if dim_unit == "in" then
        formula1 = luagimp.variables.active_file.x_resolution
    end
    if off_unit == "in" then
        formula2 = luagimp.variables.active_file.x_resolution
    end
    ...
    if dim_unit == "in" then
        formula1 = luagimp.variables.active_file.y_resolution
    end
    if off_unit == "in" then
        formula2 = luagimp.variables.active_file.y_resolution
    end
    ...
    new_layer.rendered = love.graphics.newCanvas(new_layer.width, new_layer.height)
    local fillwith_color
    love.graphics.setCanvas(new_layer.rendered)
    if new_layer.fill_with == "transparency" then
        fillwith_color = {1, 1, 1, 0}
    elseif new_layer.fill_with == "white" then
        fillwith_color = {1, 1, 1}
    elseif new_layer.fill_with == "background-color" then
        fillwith_color = luagimp.variables.background_color
    elseif new_layer.fill_with == "foreground-color" then
        fillwith_color = luagimp.variables.foreground_color
    end
    love.graphics.clear(fillwith_color)
    love.graphics.setCanvas()
    love.graphics.draw(new_layer.rendered)
